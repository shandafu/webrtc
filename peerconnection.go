...
func NewPeerConnection(configuration Configuration, pli *int) (*PeerConnection, error) {
	m := &MediaEngine{}
	...
	if err := RegisterDefaultInterceptors(m, i, pli); err != nil {
		return nil, err
	}
...
func (api *API) NewPeerConnection(configuration Configuration) (*PeerConnection, error) {
	...
	pc.interceptorRTCPReader = pc.api.interceptor.BindRTCPReader(interceptor.RTCPReaderFunc(func(in []byte, _ interceptor.Attributes) (int, interceptor.Attributes, error) {
		// log.Println("Read RTCP packet")
...
func (pc *PeerConnection) undeclaredMediaProcessor() {
	...
	go func() {
		for {
			...
			readStreamSRTCP, ssrc, err := srtcpSession.AcceptStream()
			// log.Println("peerconnection ssrc: ", uint32(ssrc))
			if err != nil {
				log.Println("Failed to accept RTCP ", err)
			...
			b := make([]byte, 1500)
			i, _ := readStreamSRTCP.Read(b)
			// log.Println("peerconnection read packet size: ", i)

			if _, _, err = pc.interceptorRTCPReader.Read(b[:i], nil); err != nil {
				panic(err)
			}
...
func (pc *PeerConnection) newTransceiverFromTrack(direction RTPTransceiverDirection, track TrackLocal) (t *RTPTransceiver, err error) {
	...
	pc.InterceptorRTPWriter = pc.api.interceptor.BindLocalStream(streamInfo, interceptor.RTPWriterFunc(func(header *rtp.Header, payload []byte, attributes interceptor.Attributes) (int, error) {
