...
func (s *TrackLocalStaticRTP) WriteRTP(p *rtp.Packet, rtpWriter interceptor.RTPWriter) error {
	...
	*packet = *p
	return s.writeRTP(packet, rtpWriter)
}
...
func (s *TrackLocalStaticRTP) writeRTP(p *rtp.Packet, rtpWriter interceptor.RTPWriter) error {
	...
	for _, b := range s.bindings {
		...
		if rtpWriter != nil {
			n, err := rtpWriter.Write(&p.Header, p.Payload, nil)
			if err != nil {
				log.Println("rtpWriter interceptor fail: ", err, n)
			}
			// log.Println("rtpWriter interceptor success: ", n)
		}
...
func (s *TrackLocalStaticRTP) Write(b []byte) (n int, err error) {
	...
	return len(b), s.writeRTP(packet, nil)
}
...
// all PeerConnections. The error message will contain the ID of the failed
// PeerConnections so you can remove them
func (s *TrackLocalStaticSample) WriteSample(sample media.Sample, rtpWriter interceptor.RTPWriter, IsKeyFrame bool) error {
	s.rtpTrack.mu.RLock()
	p := s.packetizer
	...
	for _, p := range packets {
		p.Header.ExtensionProfile = keyFrame
		if err := s.rtpTrack.WriteRTP(p, rtpWriter); err != nil {
			writeErrs = append(writeErrs, err)
		}
	}
